...
function timeSpanToString(startDate, endDate) {
  let timeGap = endDate.getTime() - startDate.getTime();
  const hours = Math.floor(timeGap / 3600000);
  timeGap %= 3600000;
  const minutes = Math.floor(timeGap / 60000);
  timeGap %= 60000;
  const seconds = Math.floor(timeGap / 1000);
  timeGap %= 1000;
  return `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}.${String(timeGap).padStart(3, '0')}`;
}
