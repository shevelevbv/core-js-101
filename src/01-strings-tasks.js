...
function encodeToRot13(str) {
  ...
  str.split('').forEach((letter) => {
    if (letter.charCodeAt(0) >= 'A'.charCodeAt(0) && letter.charCodeAt(0) <= 'Z'.charCodeAt(0)) {
      ind = letter.charCodeAt(0) + 13;
      if (ind > 'Z'.charCodeAt(0)) {
        ind -= 26;
      }
      result += String.fromCharCode(ind);
...
function getCardId(value) {
  ...
  switch (value.slice(0, -1)) {
    ...
    default: number = parseInt(value.slice(0, -1), 10);
  }
  switch (value[value.length - 1]) {
    case '♣': return number - 1;
    case '♦': return number + 13 - 1;
    case '♥': return number + 2 * 13 - 1;
    case '♠': return number + 3 * 13 - 1;
    default: return 0;
  }
}
